...
def process_single_molecule(smi: str) -> Dict[str, Any]:
    ...
    ----------
    smi : str
        The raw SMILES string generated by the model.

    Returns
    -------
    Dict[str, Any]
        A dictionary containing the standardized SMILES, molecular
        properties, fingerprints, and validity status.
    """
    invalid_dict = {
        "valid": False,
        "smi": smi,
    ...
    scaffold_fp = mol_to_fp(scaffold_mol)
...
def apply_diversity_filter(
    ...
    """
    REINVENT4-style Tanimoto diversity filter using optimized Numpy operations.
    """
    final_scores = []
    ...
    for i, data in enumerate(processed_data):
        ...
        if global_memory:
            memory_items = list(global_memory.items())

            scaffold_sum = scaffold_fp.sum()

            for start in range(0, len(memory_items), CHUNK_SIZE):
                chunk = memory_items[start : start + CHUNK_SIZE]
                chunk_fps = [item[1]["fp"] for item in chunk]

                chunk_matrix = np.stack(chunk_fps)
                intersection = np.dot(chunk_matrix, scaffold_fp)
                union = chunk_matrix.sum(axis=1) + scaffold_sum - intersection
                sims = intersection / (union + 1e-8)  # Add to prevent div by 0

                for j, sim in enumerate(sims):
                    if sim >= min_similarity:
                        best_match_smi = chunk[j][0]
                        break
